...
function buzzGradientSampleImage(isDark: boolean): string {
  ...
  const foreground = `hsl(${readToken(
    "--foreground",
    isDark ? "0 0% 98%" : "0 0% 9%",
  )})`;
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 382 200"><defs><linearGradient id="g" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="${top}"/><stop offset="1" stop-color="${bottom}"/></linearGradient></defs><rect width="382" height="200" fill="url(#g)"/><rect x="76" y="64" width="72" height="72" rx="22" fill="${foreground}" opacity="0.5"/><rect x="168" y="76" width="96" height="18" rx="9" fill="${foreground}" opacity="0.38"/><rect x="168" y="106" width="138" height="18" rx="9" fill="${foreground}" opacity="0.28"/></svg>`;
