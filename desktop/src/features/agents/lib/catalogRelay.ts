...
/**
 * Whether a relay event carries a valid signature over its own contents.
 *
 * The publisher controls every catalog head end to end, so authorship and
 * integrity cannot be trusted until the signature is checked. Both catalog
 * kinds flow through [`sharedCatalogHeads`], which verifies here before
 * claiming a coordinate — a forged newer head must not shadow an older, valid
 * one, and a tampered body must not project as a valid definition.
 */
...
export function sharedCatalogHeads(
  ...
  for (const event of sorted) {
    ...
    // Verify before claiming the coordinate: an unsigned or forged head must
    // neither project as valid nor shadow an older, validly signed head.
...
export async function fetchCatalogEvents(kind: number): Promise<RelayEvent[]> {
  ...
  for (let page = 0; page < MAX_CATALOG_PAGES; page += 1) {
    ...
    for (const event of events) {
      // Verify inside the walk: an invalid event must neither be collected nor
      // advance the `until` cursor, or a forged low-`created_at` head could
      // truncate discovery for everyone.
      if (!eventHasValidSignature(event)) continue;
