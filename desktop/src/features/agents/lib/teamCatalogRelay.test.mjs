...
// Catalog projection now runs through the shared signature gate (the #4220
// hardening ported into `catalogRelay.ts`), so every discoverable head must
// carry a valid signature over its own contents. `extraTags` are folded in
// before signing so structural-rejection fixtures (two `d` tags, a second
// `shared` tag) stay validly signed and exercise the structural rule itself
// rather than being masked by a broken signature.
function teamEvent({
  createdAt = 1,
  owner = ALICE,
  teamDTag = "squad",
...
test("test_shared_team_projection_is_discoverable_with_its_members", () => {
  ...
  const publications = teamCatalogPublicationsFromEvents([event]);
  ...
  assert.equal(publications[0].eventId, event.id);
...
test("test_only_an_exact_shared_true_tag_opts_a_team_into_discovery", () => {
  ...
  const duplicate = teamEvent({
    createdAt: 5,
    extraTags: [["shared", "true"]],
  });
...
test("test_two_d_tags_make_a_head_unaddressable", () => {
  const ambiguous = teamEvent({ extraTags: [["d", "other-squad"]] });
