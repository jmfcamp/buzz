...
fn prepare_appkit_for_panel(mtm: MainThreadMarker) {
    let app = NSApplication::sharedApplication(mtm);
    let _ = app.setActivationPolicy(NSApplicationActivationPolicy::Regular);
    NSApplication::activate(&app);
}

/// `+[NSOpenPanel openPanel]` can return nil. The typed objc2 binding panics
/// on that, so retain the raw return instead (tauri-apps/tauri#13047).
fn try_open_panel(mtm: MainThreadMarker) -> Option<Retained<NSOpenPanel>> {
    prepare_appkit_for_panel(mtm);
    unsafe {
...
fn try_save_panel(mtm: MainThreadMarker) -> Option<Retained<NSSavePanel>> {
    prepare_appkit_for_panel(mtm);
    unsafe {
        let ptr: *mut NSSavePanel = msg_send![NSSavePanel::class(), savePanel];
        Retained::retain(ptr)
    }
}
...
impl Panel {
    pub fn build_pick_file(opt: &FileDialog, mtm: MainThreadMarker) -> Option<Self> {
        let panel = try_open_panel(mtm)?;
        ...
        Some(Self::new(Retained::into_super(panel), opt.parent.as_ref()))
    }

    pub fn build_save_file(opt: &FileDialog, mtm: MainThreadMarker) -> Option<Self> {
        let panel = try_save_panel(mtm)?;
        ...
        Some(Self::new(panel, opt.parent.as_ref()))
    }

    pub fn build_pick_folder(opt: &FileDialog, mtm: MainThreadMarker) -> Option<Self> {
        let panel = try_open_panel(mtm)?;
    ...
    pub fn build_pick_folders(opt: &FileDialog, mtm: MainThreadMarker) -> Option<Self> {
        let panel = try_open_panel(mtm)?;
    ...
    pub fn build_pick_files(opt: &FileDialog, mtm: MainThreadMarker) -> Option<Self> {
        let panel = try_open_panel(mtm)?;
