...
#[cfg(target_os = "macos")]
use objc2_app_kit::{
  NSApplication, NSApplicationActivationPolicy, NSModalResponse, NSModalResponseOK, NSOpenPanel,
  NSWindowDelegate,
};
    ...
    fn run_file_upload_panel(
      ...
      unsafe {
        if let Some(mtm) = MainThreadMarker::new() {
          // Tahoe + ad-hoc/unsigned builds often refuse the file-dialog XPC
          // service when the process is not a regular activating app (crash
          // reports show Role: Background). Promote and activate first.
          let app = NSApplication::sharedApplication(mtm);
          let _ = app.setActivationPolicy(NSApplicationActivationPolicy::Regular);
          NSApplication::activate(&app);

          // NSOpenPanel::openPanel() can return nil (code-signature mismatch
          // after in-place update, unsigned local .app, system wake). The
          // typed objc2 binding panics on nil, so retain the raw return and
          // cancel the WebKit upload instead of aborting.
          // See tauri-apps/wry#1716 and tauri-apps/tauri#13047.
